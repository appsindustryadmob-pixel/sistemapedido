        ...
        <div class="col-md-8 col-sm-8 col-xs-12">
            ...
            <div>
                ...
                if ($esAdmin) {
                    ...
                    $sql = "SELECT * FROM pedidos WHERE nombre_usuario = '$nombreUsuario' ORDER BY fecha_creacion DESC";
                }
...
th, td {
    ...
    white-space: nowrap;
    max-width: 230px; /* Ajusta el ancho máximo de las celdas según sea necesario */
}
...
@media only screen and (max-width: 600px) {
    ...
    table td {
        ...
        font-size: 0.8em;
        text-align: right;
        max-width: 330px; /* Elimina la limitación de ancho máximo en dispositivos pequeños */
    }
...
/* Estilo para permitir que el comentario de entrega se desborde y se muestre en varias líneas */
    ...
    max-width: none;
                ...
                <body>
                    ...
                    if ($resultado->num_rows > 0) {
                        echo '<table>';
                        echo '<thead><tr><th>ID</th><th>Fecha del Pedido</th><th>Fecha de Entrega</th><th>Productos</th><th>Comentario</th><th>Estado del Pedido</th>';
                        ...
                        while ($fila = $resultado->fetch_assoc()) {
                            ...
                            echo '<td data-label="Fecha del Pedido">' . date('Y-m-d', strtotime($fila['resumen_fecha'])) . '</td>';
                            echo '<td data-label="Fecha de Entrega">' . ($fila['fecha_entrega_confirmada'] ?: 'No confirmada') . '</td>';
                            echo '<td data-label="Productos">' . nl2br($fila['lista_productos']) . '</td>';
                            echo '<td data-label="Comentario">' . $fila['comentario'] . '</td>';
                            echo '<td data-label="Estado del Pedido">' . ($fila['enviado'] ? 'Entregado' : 'Pendiente') . '</td>';
                            ...
                            if (!$fila['enviado']) {
                                echo '<button onclick="confirmarEnvio(' . $fila['id'] . ')">Confirmar</button>';
                    ...
                    <script>
                        ...
                        function imprimirLista(pedidoId) {
                            ...
                            fetch('obtener_info_pedido.php?pedidoId=' + pedidoId)
                                .then(response => response.json())
                                .then(data => {
                                    // Construye el contenido HTML para imprimir
                                    var contenidoHTML = `
                                        <div>
                                            <p>SISTEMAS DE PEDIDOS</p>
                                            <p>Tienda</p>
